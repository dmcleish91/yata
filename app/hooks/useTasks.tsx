...
export function useTasks() {
  const { data: tasksRaw, error, isLoading } = useSWR(TaskAPIEndpoints.TASKS, fetchData<Task[]>);
  ...
  async function handleAddTask(e: React.FormEvent): Promise<void> {
    ...
    if (editTaskID) {
      ...
      try {
        const response = await editResource<typeof payload, Task>(TaskAPIEndpoints.EDIT_TASKS, payload);
      ...
      try {
        const response = await createResource<typeof payload, Task>(TaskAPIEndpoints.TASKS, payload);
  ...
  async function handleDeleteTask(id: string): Promise<void> {
    const previousTasks = tasks;
    mutate(TaskAPIEndpoints.TASKS, (currentTasks: Task[] = []) => currentTasks.filter((t) => t.task_id !== id), false);
    try {
      const wasSuccessful = await deleteResource(TaskAPIEndpoints.DELETE_TASKS(id));
      if (!wasSuccessful) {
