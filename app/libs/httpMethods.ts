...
export async function createResource<T, R>(url: string, payload: T): Promise<APIResponse<R>> {
  const response = await ax.post<APIResponse<R>>(url, payload);
  return response.data;
}

export async function editResource<T, R>(url: string, payload: T, method: HttpMethod = HttpMethod.POST): Promise<APIResponse<R>> {
  let response;
